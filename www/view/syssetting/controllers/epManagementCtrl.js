'use strict';
module.exports = angular.module("app.sysSetting").controller("epManagementCtrl",
    ['$scope','tooltip','modifyById','arrayAction','httpServer','objectAction','scrollObj',
        function($scope,tooltip,modifyById,arrayAction,httpServer,objectAction,scrollObj) {
            ...
            $scope.btnAction={
                ...
                addPage:function(){
                    ...
                    if($scope.btnAction.showAddOrModify) {
                        var d = httpServer.postData('/org/authList', {"entity": {}});
                        d.then(function (res) {
                            if (res != 'error') {
                                console.log(res);
                                $scope.addModifyReqParam.volist = res;
                            }
                        });
                    }
                },
                ...
                modifyPage:function(id){
                    if(!chkItem()) return;
                    var d = httpServer.postData('/org/view', {"entity":{"orgId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            console.log(res);
                            //绑定数据并跳转到修改页面
                ...
                takeEffect:function(id,status){
                    ...
                    if(obj.entity.orgStatus!==status){
                        tooltip('需选择状态为打开的项目进行生效！');
                        return;
                    };
                    var d = httpServer.postData('/org/enable', {"entity":{"orgId":id}});
                    d.then(function(res){
                        if(res!='error'){
                ...
                loseEffect:function(id,status){
                    ...
                    var d = httpServer.postData('/org/disable', {"entity":{"orgId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            $scope.searchformAction.search();
                            $scope.selectById={
                ...
                cancel:function(id,status){
                    ...
                    var d = httpServer.postData('/org/cancel', {"entity":{"orgId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            $scope.searchformAction.search();
                            $scope.selectById={
            ...
            function checkout(item){
                if(item.orgNo.$invalid){//判断是否为空
                    scrollObj.scrollTo('input#orgNo');
                ...
                if(item.orgName.$invalid){ //判断是否为空
                    scrollObj.scrollTo('input#orgName');
                    $scope.addModifyBtnAction.tooltipIsOpen=true;
                    setTimeout(function(){
                        $scope.addModifyBtnAction.tooltipIsOpen=false;// 消失提示和弹出
                        $scope.$apply();
                ...
                if(item.contactAddress.$invalid){ //判断是否为空
                    scrollObj.scrollTo('input#contactAddress');
                    $scope.addModifyBtnAction.tooltipIsOpen=true;
                    setTimeout(function(){
                        $scope.addModifyBtnAction.tooltipIsOpen=false;// 消失提示和弹出
                        $scope.$apply();
            ...
            $scope.addModifyBtnAction={
                ...
                temporarilySave:function(formName){
                    ...
                    //通过是否存在id来判断新增还是修改
                    if($scope.addModifyReqParam.entity.orgId==null || $scope.addModifyReqParam.entity.orgId==''){
                        httpServer.postData('/org/add',$scope.addModifyReqParam).then(function(res){
                            if (res!='error' && res.entity!=null && res.entity.orgId != null){
                                console.log(res);
                                $scope.addModifyReqParam.entity.orgId = res.entity.orgId;
                                tooltip('暂存成功！');
                            } else if(res.entity == null || res.entity.orgId == null){
                                tooltip('暂存失败！');
                            }
                        })
                    } else {
                        httpServer.postData('/org/update',$scope.addModifyReqParam).then(function(res){
                            if (res!='error'){
                                tooltip('暂存成功！');
                            }
                        })
