'use strict';
module.exports = angular.module("app.sysSetting").controller("usersMgmtCtrl",
        ...
        function($scope,tooltip,modifyById,arrayAction,httpServer,objectAction,scrollObj) {
            ...
            $scope.btnAction={
                ...
                modifyPage:function(id){
                    if(!chkItem()) return;
                    var d = httpServer.postData('/account/view', {"entity":{"accountId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            console.log(res);
                            //绑定数据并跳转到修改页面
                ...
                takeEffect:function(id,status){
                    ...
                    if(obj.entity.accountStatus!==status){
                        tooltip('需选择状态为打开的项目进行生效！');
                        return;
                    };
                    var d = httpServer.postData('/account/enable', {"entity":{"accountId":id}});
                    d.then(function(res){
                        if(res!='error'){
                ...
                loseEffect:function(id,status){
                    ...
                    var d = httpServer.postData('/account/disable', {"entity":{"accountId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            $scope.searchformAction.search();
                            $scope.selectById={
                ...
                cancel:function(id,status){
                    ...
                    var d = httpServer.postData('/account/cancel', {"entity":{"accountId":id}});
                    d.then(function(res){
                        if(res!='error'){
                            $scope.searchformAction.search();
                            $scope.selectById={
            ...
            $scope.detailAction={
                ...
                addList:function(item){
                    ...
                    if($scope.subObject.roleId==null || $scope.subObject.roleId==''){//判断是否为空
                        tooltip('角色不能为空！');
                        return false;
                    };
                    //TODO 包装系数不能为空
                    ...
                    var _canPush=true;
                    angular.forEach($scope.addModifyReqParam.list, function(data,index){
                        if(data.roleId==$scope.subObject.roleId){
                            _canPush=false;
                            tooltip('不允许重复加入！');
                            return;
                        }
            ...
            function checkout(form){
                ...
                if($scope.addModifyReqParam.entity.accountPwd != $scope.addModifyReqParam.entity.accountPwd2) {
                    tooltip('用户密码跟重复密码不一致！');
                    return false;
                };
                return true;
            };
            $scope.addModifyBtnAction={
                ...
                temporarilySave:function(formName){
                    ...
                    //通过是否存在id来判断新增还是修改
                    if($scope.addModifyReqParam.entity.accountId==null || $scope.addModifyReqParam.entity.accountId==''){
                        httpServer.postData('/account/add',$scope.addModifyReqParam).then(function(res){
                            if (res!='error' && res.entity!=null && res.entity.accountId != null){
                                console.log(res);
                                $scope.addModifyReqParam = res;
                                tooltip('暂存成功！');
                            } else if(res.entity == null || res.entity.accountId == null){
                                tooltip('暂存失败！');
                            }
                        })
                    } else {
                        httpServer.postData('/account/update',$scope.addModifyReqParam).then(function(res){
                            if (res!='error'){
                                tooltip('暂存成功！');
                            }
                        })
            ...
            //列表页的查询参数begin
            //绑定查询参数，不直接绑定分页指令，防止数据改变时直接从后台查询数据
            $scope.queryReqParam4Role={ //个字段的顺序和前端显示顺序要一致 方便下步复用
                "entity":{}
            };
            //空参数，方便重置时复制
            var _queryReqParam4Role={
                "entity":{}
            };
            //绑定分页指令数据，用于用户点击查询按钮时候从后台查询数据
            $scope.queryParam4Role={
                "entity":{}
            };
            //列表页的查询参数end
            //列表页的【查询，重置】按钮触发的事件
            $scope.selectForm4Role={
                search:function(){
                    //从后台查询数据，与绑定的page指令进行双向数据绑定即可
                    //不能直接把queryReqParam拷贝到queryParam，否则不会从后台查询数据
                    var _pageModelQuery={};
                    angular.copy($scope.queryReqParam4Role, _pageModelQuery);
                    $scope.queryParam4Role = _pageModelQuery;
                },
                reset:function(){
                    angular.copy(_queryReqParam4Role, $scope.queryReqParam4Role);
                },
